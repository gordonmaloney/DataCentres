...
const FetchTarget = ({
  ...
  // Attempt to avoid re-fetching if data is already provided via props (e.g. from previous page)
  const hasData = adminDivisions && (adminDivisions.ward || adminDivisions.constituency);
  const initialRef = (hasData && postcode) ? postcode.trim() : "";

  // Keep track of the last postcode we successfully fetched to avoid duplicate calls
  const lastFetchedRef = useRef(initialRef);
  ...
  const fetchPostcodeData = async (raw) => {
    ...
    if (!isValidPostcode(trimmedPostcode)) {
      setInvalidPC(true);
      setAdminDivisions({ ward: "", constituency: "", council: "", scotConstituency: "" });
      setErrorMessage(
      ...
      if (uk_data.result.country === "Scotland") {
        ...
        if (
          ...
          setAdminDivisions({
            ...
            council: uk_data.result.admin_district || "",
            scotConstituency:
